...
const Navbar = () => {
    ...
    return (
        ...
        <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-3 md:px-12 lg:px-18 xl:px-25 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-3" : "py-4 md:py-4"}`}>

            {/* Logo */}
            <a href="/" className="flex items-center gap-2">
                <img src={logo} className={`h-24 w-24 transition-transform duration-300 ${isScrolled ? "scale-90" : "scale-100"}`} />
            </a>
                ...
                {user ?
                    (<UserButton>
                        <UserButton.MenuItems>
                            <UserButton.Action labelIcon={<FaBook />} label='My Booking' onClick={() => navigate("/my-bookings")} />
                        </UserButton.MenuItems>
                    </UserButton>) :
